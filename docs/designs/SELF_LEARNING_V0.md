...
Structured, typed, confidence-scored learnings that every fstack skill can read and
write. The goal: after 20 sessions on the same codebase, fstack knows every
...
YC founders building with AI. The people who run fstack on real codebases 20+ times
...
fstack has four distinct persistence layers. They share storage patterns
(JSONL in `~/.fstack/projects/$SLUG/`) but serve different purposes:
...
What shipped:
- Learnings persistence at `~/.fstack/projects/{slug}/learnings.jsonl`
...
- E1: Adaptive specialist gating, auto-skip specialists with 0-finding track record.
  Store per-project hit rates via fstack-learnings-log. User can force with --security etc.
...
- E5: Cross-review finding dedup, read fstack-review-read for prior review entries.
  Suppress findings matching a prior user-skipped finding.
- E7: Specialist performance tracking, log per-specialist metrics via fstack-review-log.
...
- Session timeline: every skill auto-logs start/complete events to
  `~/.fstack/projects/$SLUG/timeline.jsonl`. Local-only, never sent anywhere,
...
- Predictive skill suggestion: if your last 3 sessions follow a pattern
  (review, ship, review), fstack suggests what you probably want next.
...
- Timeline binaries: `bin/fstack-timeline-log` and `bin/fstack-timeline-read`.
