...
fstack projects... CLAUDE.md works as-is.
...
### Critical env vars for fstack integration
...
Without native `.slate/` publishing, fstack skills vanish when this flag is set.
...
fstack skills already work in Slate via the `.claude/skills/` fallback path.
No changes needed for basic functionality. Users who install fstack for Claude Code
...
- Host names duplicated in `bin/fstack-platform-detect`, `bin/fstack-uninstall`, `bin/dev-setup`
...
- `lib/worktree.ts` only copies `.agents/`, not `.slate/` — E2E tests in worktrees won't
  have Slate skills
- `bin/fstack-uninstall` doesn't know about `.slate/`
- `bin/dev-setup` doesn't wire `.slate/` for contributor dev mode
- `bin/fstack-platform-detect` doesn't detect Slate
