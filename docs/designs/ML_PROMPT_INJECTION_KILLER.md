...
**Date:** 2026-03-28
**CEO Plan:** ~/.fstack/projects/garrytan-fstack/ceo-plans/2026-03-28-sidebar-prompt-injection-defense.md

## The Problem

The fstack Chrome extension sidebar gives Claude bash access to control the browser.
...
```typescript
// Public API -- any fstack component can call these
...
```json
// ~/.fstack/security/attempts.jsonl
{
...
```
AskUserQuestion:
  "fstack just blocked a prompt injection attempt from {domain}. These detections
   are rare and valuable for improving defenses for all fstack users. Can we
...
2. Cache to ~/.fstack/models/browsesafe-bench/ (not re-downloaded in CI)
...
- fstack becomes the only open source tool with native-speed prompt injection detection.
...
**Limitation:** macOS-only (Linux would need OpenBLAS FFI). But fstack already
...
- [ ] Implement `loadModel()` with download-on-first-use to ~/.fstack/models/
