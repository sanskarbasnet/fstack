...
When Claude controls your real browser via CDP (fstack `$B connect`), you look at two
windows: **Conductor** (to see Claude's thinking) and **Chrome** (to see Claude's actions).

fstack's Chrome extension Side Panel shows browse activity — every command, result,
...
A "Session" tab in the fstack Chrome extension Side Panel that shows:
...
## What fstack builds (extension side)
