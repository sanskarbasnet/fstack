...
fstack already produces valuable artifacts that survive on disk: CEO plans,
...
survive compaction. Because nobody else has fstack's artifact architecture.

## The Insight

fstack already writes structured artifacts to `~/.fstack/projects/$SLUG/`:
- CEO plans: `ceo-plans/`
...
the agent checks `~/.fstack/projects/$SLUG/` for recent plans, reviews, and
...
Plan: ~/.fstack/projects/$SLUG/checkpoints/latest.md"
...
- Not a replacement for Claude's built-in compaction (that handles session
  state; we handle fstack artifacts)
