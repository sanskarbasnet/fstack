# Design: fstack Visual Design Generation (`design` binary)
...
**Screenshot consent (first-time only for $D evolve):** "This will send a screenshot of your live site to OpenAI for design evolution. [Proceed] [Don't ask again]" Stored in ~/.fstack/config.yaml as design_screenshot_consent.
...
- Exploration variants go to `/tmp/fstack-mockups-{session}/` (ephemeral, not committed)
...
- Fallback: if not in a git repo, save to `/tmp/fstack-mockup-{timestamp}.png`
...
```
Note: Codex runtime setup (`setup` script) must also export `FSTACK_DESIGN` env var, similar to how `FSTACK_BROWSE` is set.

**`$D` resolution bash block** (generated by `{{DESIGN_SETUP}}`):
```bash
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
D=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.claude/skills/fstack/design/dist/design" ] && D="$_ROOT/.claude/skills/fstack/design/dist/design"
[ -z "$D" ] && D=~/.claude/skills/fstack/design/dist/design
if [ -x "$D" ]; then
  echo "DESIGN_READY: $D"
else
...
| `scripts/resolvers/preamble.ts` | Add `FSTACK_DESIGN` env var export for Codex host |
...
1. Read `~/.fstack/openai.json` → `{ "api_key": "sk-..." }` (file permissions 0600)
...
3. If neither exists → guided setup flow:
   ...
   - Write to `~/.fstack/openai.json` with 0600 permissions
...
- Symlinked via existing `~/.claude/skills/fstack/` install path
...
- Auth module (read ~/.fstack/openai.json, fallback to env var, guided setup flow)
...
- Add FSTACK_DESIGN env var export to `scripts/resolvers/preamble.ts` (Codex host)
