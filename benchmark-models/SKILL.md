...
version: 1.0.0
description: |
  Cross-model benchmark for fstack skills. Runs the same prompt through Claude,
  GPT (via Codex CLI), and Gemini side-by-side — compares latency, tokens, cost,
  and optionally quality via LLM judge. Answers "which model is actually best
  for this skill?" with data instead of vibes. Separate from /benchmark, which
  measures web page performance. Use when: "benchmark models", "compare models",
  "which model is best for X", "cross-model comparison", "model shootout". (fstack)
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"benchmark-models","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.fstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/fstack/bin/fstack-timeline-log '{"skill":"benchmark-models","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
You are running the `/benchmark-models` workflow. Wraps the `fstack-model-benchmark` binary with an interactive flow that picks a prompt, confirms providers, previews auth, and runs the benchmark.

Different from `/benchmark` — that skill measures web page performance (Core Web Vitals, load times). This skill measures AI model performance on fstack skills or arbitrary prompts.
...
```bash
BIN="$HOME/.claude/skills/fstack/bin/fstack-model-benchmark"
[ -x "$BIN" ] || BIN=".claude/skills/fstack/bin/fstack-model-benchmark"
[ -x "$BIN" ] || { echo "ERROR: fstack-model-benchmark not found. Run ./setup in the fstack install dir." >&2; exit 1; }
echo "BIN: $BIN"
```

If not found, stop and tell the user to reinstall fstack.
...
- **Options:**
  - A) Benchmark one of my fstack skills (we'll pick which skill next). Completeness: 10/10.
...
If A: list top-level fstack skills that have SKILL.md files (from `find . -maxdepth 2 -name SKILL.md -not -path './.*'`), ask the user to pick one via a second AskUserQuestion. Use the picked SKILL.md path as the prompt file.
...
- **Options:**
  - A) Save to `~/.fstack/benchmarks/<date>-<skill-or-prompt-slug>.json`. Completeness: 10/10.
