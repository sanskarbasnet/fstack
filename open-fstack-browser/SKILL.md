...
description: |
  ...
  Use when asked to "open fstack browser", "launch browser", "connect chrome",
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"open-fstack-browser","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.fstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/fstack/bin/fstack-timeline-log '{"skill":"open-fstack-browser","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/fstack/bin/fstack-question-log '{"skill":"open-fstack-browser","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
# /open-fstack-browser — Launch GStack Browser
...
if [ -f "$(git rev-parse --show-toplevel 2>/dev/null)/.fstack/browse.json" ]; then
  _OLD_PID=$(cat "$(git rev-parse --show-toplevel)/.fstack/browse.json" 2>/dev/null | grep -o '"pid":[0-9]*' | grep -o '[0-9]*')
  [ -n "$_OLD_PID" ] && kill "$_OLD_PID" 2>/dev/null || true
  sleep 1
  [ -n "$_OLD_PID" ] && kill -9 "$_OLD_PID" 2>/dev/null || true
  rm -f "$(git rev-parse --show-toplevel)/.fstack/browse.json"
fi
# Clean Chromium profile locks (can persist after crashes)
_PROFILE_DIR="$HOME/.fstack/chromium-profile"
...
- The fstack sidebar extension auto-loaded via `launchPersistentContext`
...
The `connect` command auto-discovers the extension from the fstack install
...
```bash
cat "$(git rev-parse --show-toplevel 2>/dev/null)/.fstack/browse.json" 2>/dev/null | grep -o '"port":[0-9]*' | grep -o '[0-9]*'
```
...
_EXT_PATH=""
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
[ -n "$_ROOT" ] && [ -f "$_ROOT/.claude/skills/fstack/extension/manifest.json" ] && _EXT_PATH="$_ROOT/.claude/skills/fstack/extension"
[ -z "$_EXT_PATH" ] && [ -f "$HOME/.claude/skills/fstack/extension/manifest.json" ] && _EXT_PATH="$HOME/.claude/skills/fstack/extension"
...
> Chrome is launched with fstack control. You should see Playwright's Chromium
...
>    already show the fstack icon if the extension loaded successfully
> 2. Click the **puzzle piece** → find **fstack browse** → click the **pin icon**
> 3. Click the pinned **fstack icon** in the toolbar
...
> 2. Look for **"fstack browse"** — it should be listed and enabled
...
>
> If the Side Panel badge stays gray (disconnected), click the fstack icon
...
> - Run any fstack skill (`/qa`, `/design-review`, `/benchmark`) and watch
