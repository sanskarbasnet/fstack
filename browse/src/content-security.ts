 ...
 * Marks hidden elements with data-fstack-hidden attribute.
  ...
  return page.evaluate((ariaPatterns: string[]) => {
    ...
    for (const el of elements) {
      if (el instanceof HTMLElement) {
        ...
        if (isHidden) {
          el.setAttribute('data-fstack-hidden', 'true');
...
export async function getCleanTextWithStripping(page: Page | Frame): Promise<string> {
  return page.evaluate(() => {
    ...
    clone.querySelectorAll('[data-fstack-hidden]').forEach(el => el.remove());
...
/**
 * Clean up data-fstack-hidden attributes from the page.
 * Should be called after extraction is complete.
 */
export async function cleanupHiddenMarkers(page: Page | Frame): Promise<void> {
  await page.evaluate(() => {
    document.querySelectorAll('[data-fstack-hidden]').forEach(el => {
      el.removeAttribute('data-fstack-hidden');
    });
  });
}
