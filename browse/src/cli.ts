/**
 * fstack CLI — thin wrapper that talks to the persistent server
 *
 * Flow:
 *   1. Read .fstack/browse.json for port + token
...
function cleanupLegacyState(): void {
  // No legacy state on Windows — /tmp and `ps` don't exist, and fstack
...
async function ensureServer(): Promise<ServerState> {
  ...
  if (process.env.BROWSE_NO_AUTOSTART === '1') {
    ...
    console.error('[browse] The headed browser may have been closed. Run /open-fstack-browser to restart.');
    process.exit(1);
  }
  ...
  if (state && state.mode === 'headed' && isProcessAlive(state.pid)) {
    ...
    console.error(`[browse] Run '/open-fstack-browser' to restart.`);
    process.exit(1);
  }
...
/** Check if ngrok is installed and authenticated (native config or fstack env). */
function isNgrokAvailable(): boolean {
  // Check fstack's own ngrok env
  const ngrokEnvPath = path.join(process.env.HOME || '/tmp', '.fstack', 'ngrok.env');
...
async function handlePairAgent(state: ServerState, args: string[]): Promise<void> {
  ...
  if (localHost) {
    try {
      ...
      let globalRoot = `.${localHost}/skills/fstack`;
      try {
...
async function main() {
  ...
  if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
    console.log(`fstack browse — Fast headless browser for AI coding agents
