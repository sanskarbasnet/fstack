 ...
 * X_OK, case-insensitive Path/PATH on Windows). Adds the fstack-specific
 * override + arg-prefix logic on top.
 *
 * Override precedence:
 *   1. FSTACK_CLAUDE_BIN (or CLAUDE_BIN as fallback) — absolute path or
 ...
 *
 * Arg prefix:
 *   FSTACK_CLAUDE_BIN_ARGS (or CLAUDE_BIN_ARGS) prepends arguments to every
...
function parseOverrideArgs(env: NodeJS.ProcessEnv): string[] {
  const raw = env.FSTACK_CLAUDE_BIN_ARGS ?? env.CLAUDE_BIN_ARGS;
  if (!raw?.trim()) return [];
  try {
...
export function resolveClaudeCommand(
  ...
  const override = (env.FSTACK_CLAUDE_BIN ?? env.CLAUDE_BIN)?.trim();
  ...
  if (override) {
    ...
    // overrides like FSTACK_CLAUDE_BIN=wsl find the actual binary.
