 ...
 *
 * Piggybacks on ~/.fstack/analytics/skill-usage.jsonl pattern (existing
 * fstack telemetry). Hostname + aggregate counters only; no body content,
 * no agent text, no command args. Respects the user's telemetry tier
 * setting (off | anonymous | community) via fstack-config.
 *
...
function fstackHome(): string {
  return process.env.FSTACK_HOME || path.join(os.homedir(), '.fstack');
}

function analyticsDir(): string {
  return path.join(fstackHome(), 'analytics');
}
...
function isDisabled(): boolean {
  ...
  if (process.env.FSTACK_TELEMETRY_OFF === '1') {
    telemetryDisabled = true;
    return true;
  }
  // Conservative default: telemetry ON unless explicitly off. Users opt out via
  // fstack-config set telemetry off (preamble reads this; we trust the env hint).
