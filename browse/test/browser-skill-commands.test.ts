...
beforeEach(() => {
  ...
  tiers = {
    project: path.join(tmpRoot, 'project', '.fstack', 'browser-skills'),
    global: path.join(tmpRoot, 'home', '.fstack', 'browser-skills'),
    bundled: path.join(tmpRoot, 'fstack-install', 'browser-skills'),
  };
...
describe('buildSpawnEnv', () => {
  ...
  beforeEach(() => {
    ...
    process.env.FSTACK_TOKEN = 'root-token';
  ...
  it('untrusted: drops $HOME and secrets', () => {
    ...
    expect(env.FSTACK_TOKEN).toBeUndefined();
  });
  ...
  it('untrusted: injects FSTACK_PORT + FSTACK_SKILL_TOKEN', () => {
    const env = buildSpawnEnv({ trusted: false, port: 1234, skillToken: 'tok-xyz' });
    expect(env.FSTACK_PORT).toBe('1234');
    expect(env.FSTACK_SKILL_TOKEN).toBe('tok-xyz');
  });
  ...
  it('trusted: still strips FSTACK_TOKEN (defense in depth)', () => {
    const env = buildSpawnEnv({ trusted: true, port: 1234, skillToken: 'tok' });
    expect(env.FSTACK_TOKEN).toBeUndefined();
  });
  ...
  it('FSTACK_PORT/FSTACK_SKILL_TOKEN can never be overridden by parent env', () => {
    process.env.FSTACK_PORT = '99999'; // attacker-set
    process.env.FSTACK_SKILL_TOKEN = 'attacker-tok';
    const env = buildSpawnEnv({ trusted: true, port: 1234, skillToken: 'real-tok' });
    expect(env.FSTACK_PORT).toBe('1234');
    expect(env.FSTACK_SKILL_TOKEN).toBe('real-tok');
  });
});
...
describe.skipIf(SKIP_SPAWN)('spawnSkill: lifecycle', () => {
  ...
  it('untrusted spawn: FSTACK_SKILL_TOKEN visible, root env scrubbed', async () => {
    const dir = makeSkillDir(tiers.bundled, 'env-probe',
      'name: env-probe\nhost: x.com',  // trusted defaults to false
      `console.log(JSON.stringify({
        port: process.env.FSTACK_PORT,
        token: process.env.FSTACK_SKILL_TOKEN,
        home: process.env.HOME ?? null,
        gh: process.env.GITHUB_TOKEN ?? null,
        fstack: process.env.FSTACK_TOKEN ?? null,
      }));`,
    );
    const origEnv = { ...process.env };
    process.env.GITHUB_TOKEN = 'gh-secret';
    process.env.FSTACK_TOKEN = 'root';
    try {
