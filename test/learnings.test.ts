...
function runLog(input: string, opts: { expectFail?: boolean } = {}): { stdout: string; exitCode: number } {
  ...
  try {
    const stdout = execSync(`${BIN}/fstack-learnings-log '${input.replace(/'/g, "'\\''")}'`, execOpts).trim();
...
function runSearch(args: string = ''): string {
  ...
  try {
    return execSync(`${BIN}/fstack-learnings-search ${args}`, execOpts).trim();
  } catch {
    return '';
  }
}
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'fstack-learn-'));
...
describe('fstack-learnings-search', () => {
...
describe('fstack-learnings-log edge cases', () => {
...
describe('fstack-learnings-search edge cases', () => {
