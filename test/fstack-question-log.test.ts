/**
 * bin/fstack-question-log — schema validation + injection defense tests.
 */
...
const BIN = path.join(ROOT, 'bin', 'fstack-question-log');

let tmpHome: string;

beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'fstack-test-'));
});
...
describe('fstack-question-log — valid payloads', () => {
...
describe('fstack-question-log — rejected payloads', () => {
...
describe('fstack-question-log — injection defense', () => {
  const attacks = [
