...
If `PROACTIVE` is `"false"`, do not proactively suggest fstack skills AND do not
...
or invoking other fstack skills, use the `/fstack-` prefix (e.g., `/fstack-qa` instead
of `/qa`, `/fstack-ship` instead of `/ship`). Disk paths are unaffected — always use
`~/.claude/skills/fstack/[skill-name]/SKILL.md` for reading skill files.

If output shows `UPGRADE_AVAILABLE <old> <new>`: read `~/.claude/skills/fstack/fstack-upgrade/SKILL.md` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined). If `JUST_UPGRADED <from> <to>`: tell user "Running fstack v{to} (just updated!)" and continue.

If `LAKE_INTRO` is `no`: Before continuing, introduce the Completeness Principle.
Tell the user: "fstack follows the **Boil the Lake** principle — always do the complete
...
> Help fstack get better! Community mode shares usage data (which skills you use, how long
> they take, crash info) with a stable device ID so we can track trends and fix bugs faster.
> No code, file paths, or repo names are ever sent.
> Change anytime with `fstack-config set telemetry off`.
...
> How about anonymous mode? We just learn that *someone* used fstack — no unique ID,
...
> fstack can proactively figure out when you might need a skill while you work —
...
If B: run `~/.claude/skills/fstack/bin/fstack-config set routing_declined true`
Say "No problem. You can add routing rules later by running `fstack-config set routing_declined false` and re-running any skill."
...
AI makes completeness near-free. Always recommend the complete option over shortcuts — the delta is minutes with CC+fstack. A "lake" (100% coverage, all edge cases) is boilable; an "ocean" (full rewrite, multi-quarter migration) is not. Boil lakes, flag oceans.
...
`~/.fstack/analytics/` (user config directory, not project files). The skill
...
- Writing to `~/.fstack/` (config, analytics, review logs, design artifacts, learnings)
...
1. Check if the plan file already has a `## FSTACK REVIEW REPORT` section.
...
Then write a `## FSTACK REVIEW REPORT` section to the end of the plan file:
...
For Design Review: run `source <(~/.claude/skills/fstack/bin/fstack-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 3.5, but consider running /design-review for a full visual audit post-implementation." Still never block.
