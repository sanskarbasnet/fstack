...
describeIfSelected('Review Dashboard Via Attribution', ['review-dashboard-via'], () => {
  ...
  beforeAll(() => {
    ...
    // fstack-review-read reads from ~/.fstack/projects/$SLUG/$BRANCH-reviews.jsonl
    // For the test, we'll write a mock fstack-review-read script that returns our test data
    ...
    // Write a mock fstack-review-read that returns our test data
    const mockBinDir = path.join(dashDir, '.mock-bin');
    fs.mkdirSync(mockBinDir, { recursive: true });
    fs.writeFileSync(path.join(mockBinDir, 'fstack-review-read'), [
      '#!/usr/bin/env bash',
    ...
    fs.chmodSync(path.join(mockBinDir, 'fstack-review-read'), 0o755);
...
Instead of running ~/.claude/skills/fstack/bin/fstack-review-read, run this mock: ${mockBinDir}/fstack-review-read
