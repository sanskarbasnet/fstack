...
describe('Cross-skill path consistency', () => {
  ...
  test('all greptile-history write references specify both per-project and global paths', () => {
    ...
    for (const file of filesToCheck) {
      ...
      const hasBoth = (content.includes('per-project') && content.includes('global')) ||
        (content.includes('$REMOTE_SLUG/greptile-history') && content.includes('~/.fstack/greptile-history'));
  ...
  test('greptile-triage.md contains both project and global history paths', () => {
    ...
    expect(content).toContain('~/.fstack/greptile-history.md');
  });
...
describe('QA skill structure validation', () => {
  ...
  test('output structure references report directory layout', () => {
    ...
    expect(qaContent).toContain('.fstack/qa-reports/');
  });
});
...
describe('office-hours skill structure', () => {
  ...
  test('contains YC apply CTA with ref tracking', () => {
    expect(content).toContain('ycombinator.com/apply?ref=fstack');
  });
...
describe('fstack-slug', () => {
  const SLUG_BIN = path.join(ROOT, 'bin', 'fstack-slug');
  ...
  test('eval sets variables under bash with set -euo pipefail', () => {
    const result = Bun.spawnSync(
      ['bash', '-c', 'set -euo pipefail; eval "$(./bin/fstack-slug 2>/dev/null)"; echo "SLUG=$SLUG"; echo "BRANCH=$BRANCH"'],
  ...
  test('no templates or bin scripts use source process substitution for fstack-slug', () => {
    const result = Bun.spawnSync(
      ['grep', '-r', 'source <(.*fstack-slug', '--include=*.tmpl', '--include=fstack-review-*', '.'],
      { cwd: ROOT, stdout: 'pipe', stderr: 'pipe' }
    );
...
describe('Phase 8e.5 regression test generation', () => {
  ...
  test('regression test includes full attribution comment format', () => {
    ...
    expect(content).toContain('// Report: .fstack/qa-reports/');
  });
...
describe('Codex skill', () => {
  ...
  test('codex/SKILL.md contains review log persistence', () => {
    ...
    expect(content).toContain('fstack-review-log');
  });
  ...
  test('codex-host ship/review do NOT contain adversarial review step', () => {
    ...
    const shipContent = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'fstack-ship', 'SKILL.md'), 'utf-8');
    expect(shipContent).not.toContain('codex review --base');
    expect(shipContent).not.toContain('CODEX_REVIEWS');

    const reviewContent = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'fstack-review', 'SKILL.md'), 'utf-8');
...
describe('Skill trigger phrases', () => {
  ...
  // Excluded: root fstack (browser tool), fstack-upgrade (fstack-specific),
  // humanizer (text tool)
...
describe('Codex skill validation', () => {
  ...
  test('all skills (except /codex) have both Claude and Codex variants', () => {
    for (const skillDir of CLAUDE_SKILLS_WITH_TEMPLATES) {
      ...
      // Codex variant
      const codexName = skillDir.startsWith('fstack-') ? skillDir : `fstack-${skillDir}`;
  ...
  test('/claude skill is external-host-only — no Claude-host variant', () => {
    ...
    expect(fs.existsSync(path.join(AGENTS_DIR, 'fstack-claude', 'SKILL.md'))).toBe(true);
  });

  test('Codex skill names follow fstack-{name} convention', () => {
    const codexDirs = fs.readdirSync(AGENTS_DIR);
    for (const dir of codexDirs) {
      // Every directory should start with fstack
      expect(dir.startsWith('fstack')).toBe(true);
      // Root is just 'fstack', others are 'fstack-{name}'
      if (dir !== 'fstack') {
        expect(dir.startsWith('fstack-')).toBe(true);
      }
    }
...
describe('Repo mode preamble validation', () => {
  test('generated SKILL.md preamble contains REPO_MODE output', () => {
    ...
    expect(content).toContain('fstack-repo-mode');
  });
...
//
// Browser-skills are bundled in <fstack-root>/browser-skills/<name>/. Each
