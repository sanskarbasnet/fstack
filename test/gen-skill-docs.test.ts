...
const ALL_SKILLS = (() => {
  ...
  if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
    skills.push({ dir: '.', name: 'root fstack' });
  }
...
describe('gen-skill-docs', () => {
  ...
  test('generated SKILL.md contains operational self-improvement (replaced contributor mode)', () => {
    ...
    expect(content).not.toContain('fstack_contributor');
    expect(content).not.toContain('contributor-logs');
    expect(content).toContain('Operational Self-Improvement');
    expect(content).toContain('fstack-learnings-log');
    expect(content).toContain('fstack-learnings-search --limit 3');
  });
  ...
  test('generated SKILL.md contains telemetry line', () => {
    ...
    expect(content).toContain('~/.fstack/analytics');
  });
  ...
  test('slim voice section preserves the fstack voice contract', () => {
    const content = fs.readFileSync(path.join(ROOT, 'plan-eng-review', 'SKILL.md'), 'utf-8');
    const voice = extractMarkdownSection(content, '## Voice');
  ...
  test('preamble .pending-* glob is zsh-safe (uses find, not shell glob)', () => {
    for (const skill of CLAUDE_GENERATED_SKILLS) {
      ...
      expect(content).toContain("find ~/.fstack/analytics -maxdepth 1 -name '.pending-*'");
    }
  });
...
describe('REVIEW_DASHBOARD resolver', () => {
  ...
  for (const skill of REVIEW_SKILLS) {
    test(`review dashboard appears in ${skill} generated file`, () => {
      ...
      expect(content).toContain('fstack-review');
...
describe('PLAN_FILE_REVIEW_REPORT resolver', () => {
  ...
  for (const skill of REVIEW_SKILLS) {
    test(`plan file review report appears in ${skill} generated file`, () => {
      const content = fs.readFileSync(path.join(ROOT, skill, 'SKILL.md'), 'utf-8');
      expect(content).toContain('FSTACK REVIEW REPORT');
    });
  }
...
describe('Plan status footer in preamble', () => {
  test('preamble contains plan status footer', () => {
    ...
    expect(content).toContain('FSTACK REVIEW REPORT');
    expect(content).toContain('fstack-review-read');
...
describe('CODEX_SECOND_OPINION resolver', () => {
  const content = fs.readFileSync(path.join(ROOT, 'office-hours', 'SKILL.md'), 'utf-8');
  const codexContent = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'fstack-office-hours', 'SKILL.md'), 'utf-8');
  ...
  test('Codex host variant does NOT contain the Phase 3.5 resolver output', () => {
    ...
    expect(codexContent).not.toContain('fstack-codex-oh-');
  });
});
...
describe('Codex filesystem boundary', () => {
  ...
  test('codex skill has rabbit-hole detection rule', () => {
    ...
    expect(content).toContain('fstack-update-check');
  ...
  test('autoplan boundary text avoids host-specific paths for cross-host compatibility', () => {
    ...
    // autoplan template uses generic 'skills/fstack' pattern instead of host-specific
    ...
    expect(boundarySection).toContain('skills/fstack');
...
describe('Codex generation (--host codex)', () => {
  ...
  const CODEX_SKILLS = (() => {
    ...
    if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
      if (!isSymlinkLoop('fstack')) {
        skills.push({ dir: '.', codexName: 'fstack' });
      }
    }
    for (const entry of fs.readdirSync(ROOT, { withFileTypes: true })) {
      if (!entry.isDirectory() || entry.name.startsWith('.') || entry.name === 'node_modules') continue;
      ...
      const codexName = entry.name.startsWith('fstack-') ? entry.name : `fstack-${entry.name}`;
  ...
  test('root fstack bundle has OpenAI metadata for Codex skill browsing', () => {
    ...
    expect(content).toContain('display_name: "fstack"');
    expect(content).toContain('Use $fstack to locate the bundled fstack skills.');
    expect(content).toContain('allow_implicit_invocation: true');
  });

  test('externalSkillName mapping: root is fstack, others are fstack-{dir}', () => {
    // Root → fstack
    expect(fs.existsSync(path.join(AGENTS_DIR, 'fstack', 'SKILL.md'))).toBe(true);
    // Subdirectories → fstack-{dir}
    expect(fs.existsSync(path.join(AGENTS_DIR, 'fstack-review', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(AGENTS_DIR, 'fstack-ship', 'SKILL.md'))).toBe(true);
    // fstack-upgrade doesn't double-prefix
    expect(fs.existsSync(path.join(AGENTS_DIR, 'fstack-upgrade', 'SKILL.md'))).toBe(true);
    // No double-prefix: fstack-fstack-upgrade must NOT exist
    expect(fs.existsSync(path.join(AGENTS_DIR, 'fstack-fstack-upgrade', 'SKILL.md'))).toBe(false);
  });
  ...
  test('/codex skill excluded from Codex output', () => {
    expect(fs.existsSync(path.join(AGENTS_DIR, 'fstack-codex', 'SKILL.md'))).toBe(false);
    expect(fs.existsSync(path.join(AGENTS_DIR, 'fstack-codex'))).toBe(false);
  });

  test('Codex output includes Claude outside-voice skill with read-only boundary', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'fstack-claude', 'SKILL.md'), 'utf-8');
    expect(content).toContain('claude -p');
    expect(content).toContain('mktemp /tmp/fstack-claude-prompt-');
    expect(content).toContain('mktemp /tmp/fstack-claude-diff-');
    expect(content).not.toContain('/tmp/fstack-claude-diff-$$');
  ...
  test('Codex review step stripped from Codex-host ship and review', () => {
    const shipContent = fs.readFileSync(path.join(AGENTS_DIR, 'fstack-ship', 'SKILL.md'), 'utf-8');
    expect(shipContent).not.toContain('codex review --base');
    expect(shipContent).not.toContain('CODEX_REVIEWS');

    const reviewContent = fs.readFileSync(path.join(AGENTS_DIR, 'fstack-review', 'SKILL.md'), 'utf-8');
  ...
  test('multiline descriptions preserved in Codex output', () => {
    ...
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'fstack-office-hours', 'SKILL.md'), 'utf-8');
    const fmEnd = content.indexOf('\n---', 4);
    const frontmatter = content.slice(4, fmEnd);
  ...
  test('hook skills have safety prose and no hooks: in frontmatter', () => {
    const HOOK_SKILLS = ['fstack-careful', 'fstack-freeze', 'fstack-guard'];
  ...
  test('Codex preamble resolves runtime assets from repo-local or global fstack roots', () => {
    // Check a skill that has a preamble (review is a good candidate)
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'fstack-review', 'SKILL.md'), 'utf-8');
    expect(content).toContain('FSTACK_ROOT');
    expect(content).toContain('$_ROOT/.agents/skills/fstack');
    expect(content).toContain('$FSTACK_BIN/fstack-config');
    expect(content).toContain('$FSTACK_ROOT/fstack-upgrade/SKILL.md');
    expect(content).not.toContain('~/.codex/skills/fstack/bin/fstack-config get telemetry');
  });

  // ─── Path rewriting regression tests ─────────────────────────

  test('sidecar paths point to .agents/skills/fstack/review/ (not fstack-review/)', () => {
    // Regression: gen-skill-docs rewrote .claude/skills/review → .agents/skills/fstack-review
    // but setup puts sidecars under .agents/skills/fstack/review/. Must match setup layout.
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'fstack-review', 'SKILL.md'), 'utf-8');
    // Correct: references to sidecar files use fstack/review/ path
    expect(content).toContain('.agents/skills/fstack/review/checklist.md');
    // design-checklist.md is now referenced via Review Army specialist (Claude only, stripped for Codex)
    // Wrong: must NOT reference fstack-review/checklist.md (file doesn't exist there)
    expect(content).not.toContain('.agents/skills/fstack-review/checklist.md');
  });

  test('sidecar paths in ship skill point to fstack/review/ for pre-landing review', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'fstack-ship', 'SKILL.md'), 'utf-8');
    // Ship references the review checklist in its pre-landing review step
    if (content.includes('checklist.md')) {
      expect(content).toContain('.agents/skills/fstack/review/');
      expect(content).not.toContain('.agents/skills/fstack-review/checklist');
    }
  });
  ...
  test('greptile-triage sidecar path is correct', () => {
    ...
    if (content.includes('greptile-triage')) {
      expect(content).toContain('.agents/skills/fstack/review/greptile-triage.md');
      expect(content).not.toContain('.agents/skills/fstack-review/greptile-triage');
    }
  });
  ...
  test('all four path rewrite rules produce correct output', () => {
    ...
    // Rule 1: ~/.claude/skills/fstack → $FSTACK_ROOT
    expect(content).not.toContain('~/.claude/skills/fstack');
    expect(content).toContain('$FSTACK_ROOT');

    // Rule 2: .claude/skills/fstack → .agents/skills/fstack
    expect(content).not.toContain('.claude/skills/fstack');

    // Rule 3: .claude/skills/review → .agents/skills/fstack/review
  ...
  test('path rewrite rules apply to all Codex skills with sidecar references', () => {
    ...
    for (const skill of CODEX_SKILLS) {
      ...
      expect(content).not.toContain('~/.claude/skills');
      expect(content).not.toContain('.claude/skills');
      if (content.includes('fstack-config') || content.includes('fstack-update-check') || content.includes('fstack-telemetry-log')) {
        expect(content).toContain('$FSTACK_ROOT');
      }
      ...
      if (content.includes('checklist.md') && !content.includes('design-checklist.md')) {
        expect(content).not.toContain('fstack-review/checklist.md');
      }
    }
  });
  ...
  test('Claude output unchanged: review skill still uses .claude/skills/ paths', () => {
    ...
    expect(content).toContain('~/.claude/skills/fstack');
  ...
  test('Claude output unchanged: all Claude skills have zero Codex paths', () => {
    for (const skill of CLAUDE_GENERATED_SKILLS) {
      ...
      // not the fstack Codex host install path.
      if (skill.dir !== 'pair-agent' && skill.dir !== 'codex' && skill.dir !== 'autoplan') {
        expect(content).not.toContain('~/.codex/');
      }
      // fstack-upgrade legitimately references .agents/skills for cross-platform detection
      if (skill.dir !== 'fstack-upgrade') {
        expect(content).not.toContain('.agents/skills');
      }
    }
  });
  ...
  test('codex host produces empty outside voices in design-review', () => {
    const codexContent = fs.readFileSync(path.join(AGENTS_DIR, 'fstack-design-review', 'SKILL.md'), 'utf-8');
    expect(codexContent).not.toContain('Design Outside Voices');
  });

  test('codex host does not include Codex design block in ship', () => {
    const codexContent = fs.readFileSync(path.join(AGENTS_DIR, 'fstack-ship', 'SKILL.md'), 'utf-8');
...
describe('Factory generation (--host factory)', () => {
  ...
  const FACTORY_SKILLS = (() => {
    ...
    if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
      if (!isSymlinkLoop('fstack')) skills.push({ dir: '.', factoryName: 'fstack' });
    }
    for (const entry of fs.readdirSync(ROOT, { withFileTypes: true })) {
      if (!entry.isDirectory() || entry.name.startsWith('.') || entry.name === 'node_modules') continue;
      if (entry.name === 'codex') continue;
      if (!fs.existsSync(path.join(ROOT, entry.name, 'SKILL.md.tmpl'))) continue;
      const factoryName = entry.name.startsWith('fstack-') ? entry.name : `fstack-${entry.name}`;
  ...
  test('sensitive skills have disable-model-invocation', () => {
    const SENSITIVE = ['fstack-ship', 'fstack-land-and-deploy', 'fstack-guard', 'fstack-careful', 'fstack-freeze', 'fstack-unfreeze'];
  ...
  test('non-sensitive skills lack disable-model-invocation', () => {
    const NON_SENSITIVE = ['fstack-qa', 'fstack-review', 'fstack-investigate', 'fstack-browse'];
  ...
  test('/codex skill excluded from Factory output', () => {
    expect(fs.existsSync(path.join(FACTORY_DIR, 'fstack-codex', 'SKILL.md'))).toBe(false);
    expect(fs.existsSync(path.join(FACTORY_DIR, 'fstack-codex'))).toBe(false);
  });

  test('Factory keeps Codex integration blocks', () => {
    // Factory users CAN use Codex second opinions (codex exec is a standalone binary)
    const shipContent = fs.readFileSync(path.join(FACTORY_DIR, 'fstack-ship', 'SKILL.md'), 'utf-8');
  ...
  test('Factory preamble uses .factory paths', () => {
    const content = fs.readFileSync(path.join(FACTORY_DIR, 'fstack-review', 'SKILL.md'), 'utf-8');
    expect(content).toContain('FSTACK_ROOT');
    expect(content).toContain('$_ROOT/.factory/skills/fstack');
    expect(content).toContain('$FSTACK_BIN/fstack-config');
  });
});
    ...
    describe(`${hostConfig.displayName} (--host ${hostConfig.name})`, () => {
      ...
      test('no .claude/skills path leakage outside repo-root sidecar symlinks', () => {
        ...
        for (const skill of skills) {
          // Dev installs may mount the repo root at host/skills/fstack as a runtime
      ...
      test('generates Claude outside-voice skill for external hosts', () => {
        const skillMd = path.join(hostDir, 'fstack-claude', 'SKILL.md');
        expect(fs.existsSync(skillMd)).toBe(true);
        const content = fs.readFileSync(skillMd, 'utf-8');
        expect(content).toContain('claude -p');
        expect(content).toContain('--disable-slash-commands');
      ...
      if (hostConfig.generation.skipSkills?.includes('codex')) {
        test('/codex skill excluded', () => {
          expect(fs.existsSync(path.join(hostDir, 'fstack-codex', 'SKILL.md'))).toBe(false);
        });
      }
    });
  }
...
describe('setup script validation', () => {
  ...
  test('Codex install uses link_codex_skill_dirs', () => {
    ...
    expect(codexSection).not.toContain('ln -snf "$FSTACK_DIR" "$CODEX_FSTACK"');
  });
  ...
  test('setup separates install path from source path for symlinked repo-local installs', () => {
    expect(setupContent).toContain('INSTALL_FSTACK_DIR=');
    expect(setupContent).toContain('SOURCE_FSTACK_DIR=');
    expect(setupContent).toContain('INSTALL_SKILLS_DIR=');
    expect(setupContent).toContain('CODEX_FSTACK="$INSTALL_FSTACK_DIR"');
    expect(setupContent).toContain('link_codex_skill_dirs "$SOURCE_FSTACK_DIR" "$CODEX_SKILLS"');
  });
  ...
  test('Codex installs always create sidecar runtime assets for the real skill target', () => {
    ...
    expect(setupContent).toContain('create_agents_sidecar "$SOURCE_FSTACK_DIR"');
  });
  ...
  test('link_codex_skill_dirs reads from .agents/skills/', () => {
    ...
    expect(fnBody).toContain('fstack*');
  });
  ...
  test('link_claude_skill_dirs creates real directories with absolute SKILL.md symlinks', () => {
    ...
    // to ensure Claude Code discovers them as top-level skills (not nested under fstack/)
    const fnStart = setupContent.indexOf('link_claude_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('linked[@]}', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('mkdir -p "$target"');
    expect(fnBody).toContain('ln -snf "$fstack_dir/$dir_name/SKILL.md" "$target/SKILL.md"');
  });
  ...
  // T1: Sidecar skip guard — prevents .agents/skills/fstack from being linked as a skill
  test('link_codex_skill_dirs skips the fstack sidecar directory', () => {
    const fnStart = setupContent.indexOf('link_codex_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('done', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('[ "$skill_name" = "fstack" ] && continue');
  });

  // T2: Dynamic $FSTACK_ROOT paths in generated Codex preambles
  test('generated Codex preambles use dynamic FSTACK_ROOT paths', () => {
    const codexSkillDir = path.join(ROOT, '.agents', 'skills', 'fstack-ship');
    ...
    expect(content).toContain('FSTACK_ROOT=');
    expect(content).toContain('$FSTACK_BIN/');
  });
  ...
  test('setup supports --host kiro with install section and sed rewrites', () => {
    ...
    expect(setupContent).toContain('~/.kiro/skills/fstack');
  });
  ...
  test('setup supports --host opencode with install section and OpenCode skill path vars', () => {
    ...
    expect(setupContent).toContain('OPENCODE_FSTACK="$OPENCODE_SKILLS/fstack"');
  });

  test('setup installs OpenCode skills into a nested fstack runtime root', () => {
  ...
  test('create_codex_runtime_root exposes only runtime assets', () => {
    ...
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('fstack/SKILL.md');
    expect(fnBody).toContain('browse/dist');
    expect(fnBody).toContain('browse/bin');
    expect(fnBody).toContain('fstack-upgrade/SKILL.md');
    ...
    expect(fnBody).not.toContain('ln -snf "$fstack_dir" "$codex_fstack"');
  });

  test('direct Codex installs are migrated out of ~/.codex/skills/fstack', () => {
    expect(setupContent).toContain('migrate_direct_codex_install');
    expect(setupContent).toContain('$HOME/.fstack/repos/fstack');
    expect(setupContent).toContain('avoid duplicate skill discovery');
  });

  // --- Symlink prefix tests (PR #503) ---

  test('link_claude_skill_dirs applies fstack- prefix by default', () => {
    const fnStart = setupContent.indexOf('link_claude_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('linked[@]}', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('SKILL_PREFIX');
    expect(fnBody).toContain('link_name="fstack-$skill_name"');
  });
  ...
  test('link_claude_skill_dirs preserves already-prefixed dirs', () => {
    ...
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('linked[@]}', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    // fstack-* dirs should keep their name (e.g., fstack-upgrade stays fstack-upgrade)
    expect(fnBody).toContain('fstack-*) link_name="$skill_name"');
  });
  ...
  test('cleanup_old_claude_symlinks removes only fstack-pointing symlinks', () => {
    ...
    expect(fnBody).toContain('fstack-*) continue');
  });
  ...
  test('setup reads skill_prefix from config', () => {
    ...
    expect(setupContent).toContain('FSTACK_CONFIG');
  });
  ...
  test('cleanup_prefixed_claude_symlinks exists and uses readlink', () => {
    ...
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('readlink');
    expect(fnBody).toContain('fstack-$skill_name');
  });
  ...
  test('welcome message references SKILL_PREFIX', () => {
    // fstack-upgrade is always called fstack-upgrade (it's the actual dir name)
    // but the welcome section should exist near the prefix logic
    expect(setupContent).toContain('Run /fstack-upgrade anytime');
  });
});

describe('discover-skills hidden directory filtering', () => {
  test('discoverTemplates skips dot-prefixed directories', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'fstack-discover-'));
    try {
...
describe('telemetry', () => {
  test('generated SKILL.md contains telemetry start block', () => {
    ...
    expect(content).toContain('fstack-config get telemetry');
  });

  test('generated SKILL.md contains telemetry opt-in prompt', () => {
    const content = fs.readFileSync(path.join(ROOT, 'SKILL.md'), 'utf-8');
    expect(content).toContain('.telemetry-prompted');
    expect(content).toContain('Help fstack get better');
    expect(content).toContain('fstack-config set telemetry community');
    expect(content).toContain('fstack-config set telemetry anonymous');
    expect(content).toContain('fstack-config set telemetry off');
  });

  test('generated SKILL.md contains telemetry epilogue', () => {
    const content = fs.readFileSync(path.join(ROOT, 'SKILL.md'), 'utf-8');
    expect(content).toContain('Telemetry (run last)');
    expect(content).toContain('fstack-telemetry-log');
...
describe('community fixes wave', () => {
  ...
  // #594 — Discoverability: every SKILL.md.tmpl description contains "fstack"
  test('every SKILL.md.tmpl description contains "fstack"', () => {
    for (const skill of ALL_SKILLS) {
      const tmplPath = skill.dir === '.' ? path.join(ROOT, 'SKILL.md.tmpl') : path.join(ROOT, skill.dir, 'SKILL.md.tmpl');
      const content = fs.readFileSync(tmplPath, 'utf-8');
      const desc = extractDescription(content);
      expect(desc.toLowerCase()).toContain('fstack');
    }
  });
...
describe('codex commands must not use inline $(git rev-parse --show-toplevel) for cwd', () => {
  ...
  // Use Bun.Glob to avoid ELOOP from .claude/skills/fstack symlink back to ROOT
...
describe('LEARNINGS_SEARCH resolver', () => {
  ...
  for (const skill of SEARCH_SKILLS) {
    test(`${skill} generated SKILL.md contains learnings search`, () => {
      ...
      expect(content).toContain('fstack-learnings-search');
    });
  }
...
describe('gen-skill-docs prefix warning (#620/#578)', () => {
  ...
  test('warns about skill_prefix when config has prefix=true', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'fstack-prefix-warn-'));
    try {
      // Create a fake ~/.fstack/config.yaml with skill_prefix: true
      const fakeHome = tmpDir;
      const fakeFstack = path.join(fakeHome, '.fstack');
      fs.mkdirSync(fakeFstack, { recursive: true });
      fs.writeFileSync(path.join(fakeFstack, 'config.yaml'), 'skill_prefix: true\n');
      ...
      expect(output).toContain('fstack-relink');
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  });
  ...
  test('no warning when skill_prefix is false or absent', () => {
    ...
    try {
      ...
      const fakeFstack = path.join(fakeHome, '.fstack');
      fs.mkdirSync(fakeFstack, { recursive: true });
      fs.writeFileSync(path.join(fakeFstack, 'config.yaml'), 'skill_prefix: false\n');
