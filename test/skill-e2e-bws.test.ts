...
Report the exact output — either "READY: <path>" or "NEEDS_SETUP".`,
  ...
  testConcurrentIfSelected('operational-learning', async () => {
    ...
    const fstackHome = path.join(opDir, '.fstack-home');
    ...
    for (const script of ['fstack-learnings-log', 'fstack-slug']) {
      fs.copyFileSync(path.join(ROOT, 'bin', script), path.join(binDir, script));
      fs.chmodSync(path.join(binDir, script), 0o755);
    }

    // fstack-learnings-log will create the project dir automatically via fstack-slug
...
\`\`\`bash
FSTACK_HOME="${fstackHome}" ${binDir}/fstack-learnings-log '{"skill":"qa","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
\`\`\`
...
Log the operational learning now. Then say what you logged.`,
    ...
    const projectsDir = path.join(fstackHome, 'projects');
