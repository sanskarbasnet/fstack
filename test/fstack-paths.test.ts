...
const BIN = path.join(ROOT, 'bin', 'fstack-paths');
...
// helper from inside a bash block (`eval "$(~/.claude/skills/fstack/bin/fstack-paths)"`)
...
function run(env: Record<string, string | undefined>): Record<string, string> {
  ...
  if (result.status !== 0) {
    throw new Error(`fstack-paths failed (status ${result.status}): ${result.stderr}`);
  }
...
describe('fstack-paths', () => {
  test('FSTACK_HOME wins over CLAUDE_PLUGIN_DATA and HOME', () => {
    const got = run({
      FSTACK_HOME: '/tmp/explicit-state',
      CLAUDE_PLUGIN_DATA: '/tmp/plugin-data',
      HOME: '/tmp/home',
    });
    expect(got.FSTACK_STATE_ROOT).toBe('/tmp/explicit-state');
  });

  test('CLAUDE_PLUGIN_DATA wins over HOME when FSTACK_HOME unset', () => {
    const got = run({
      CLAUDE_PLUGIN_DATA: '/tmp/plugin-data',
      HOME: '/tmp/home',
    });
    expect(got.FSTACK_STATE_ROOT).toBe('/tmp/plugin-data');
  });

  test('HOME-derived state root when FSTACK_HOME and CLAUDE_PLUGIN_DATA unset', () => {
    const got = run({ HOME: '/tmp/myhome' });
    expect(got.FSTACK_STATE_ROOT).toBe('/tmp/myhome/.fstack');
  });
  ...
  test('CWD fallback when HOME also unset (container env)', () => {
    ...
    const got = run({ HOME: '' });
    expect(got.FSTACK_STATE_ROOT).toBe('.fstack');
  });

  test('PLAN_ROOT chain: FSTACK_PLAN_DIR > CLAUDE_PLANS_DIR > HOME > CWD', () => {
    expect(run({ FSTACK_PLAN_DIR: '/tmp/explicit', HOME: '/h' }).PLAN_ROOT).toBe('/tmp/explicit');
