...
// Every test in this file shells out to fstack-config + fstack-relink (bash scripts
...
function run(cmd: string, env: Record<string, string> = {}, expectFail = false): string {
  try {
    return execSync(cmd, {
      cwd: ROOT,
      env: { ...process.env, FSTACK_STATE_DIR: tmpDir, ...env },
      encoding: 'utf-8',
      timeout: 10000,
...
// Create a mock fstack install directory with skill subdirs
function setupMockInstall(skills: string[]): void {
  installDir = path.join(tmpDir, 'fstack-install');
  skillsDir = path.join(tmpDir, 'skills');
  fs.mkdirSync(installDir, { recursive: true });
  fs.mkdirSync(skillsDir, { recursive: true });

  // Copy the real fstack-config and fstack-relink to the mock install
  const mockBin = path.join(installDir, 'bin');
  fs.mkdirSync(mockBin, { recursive: true });
  fs.copyFileSync(path.join(BIN, 'fstack-config'), path.join(mockBin, 'fstack-config'));
  fs.chmodSync(path.join(mockBin, 'fstack-config'), 0o755);
  if (fs.existsSync(path.join(BIN, 'fstack-relink'))) {
    fs.copyFileSync(path.join(BIN, 'fstack-relink'), path.join(mockBin, 'fstack-relink'));
    fs.chmodSync(path.join(mockBin, 'fstack-relink'), 0o755);
  }
  if (fs.existsSync(path.join(BIN, 'fstack-patch-names'))) {
    fs.copyFileSync(path.join(BIN, 'fstack-patch-names'), path.join(mockBin, 'fstack-patch-names'));
    fs.chmodSync(path.join(mockBin, 'fstack-patch-names'), 0o755);
  }
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'fstack-relink-test-'));
});
...
describe('fstack-relink (#578)', () => {
  // Test 11: prefixed symlinks when skill_prefix=true
  test('creates fstack-* symlinks when skill_prefix=true', () => {
    setupMockInstall(['qa', 'ship', 'review']);
    // Set config to prefix mode (pass install/skills env so auto-relink uses mock install)
    run(`${path.join(installDir, 'bin', 'fstack-config')} set skill_prefix true`, {
      FSTACK_INSTALL_DIR: installDir,
      FSTACK_SKILLS_DIR: skillsDir,
    });
    // Run relink with env pointing to the mock install
    const output = run(`${path.join(installDir, 'bin', 'fstack-relink')}`, {
      FSTACK_INSTALL_DIR: installDir,
      FSTACK_SKILLS_DIR: skillsDir,
    });
    // Verify fstack-* symlinks exist
    expect(fs.existsSync(path.join(skillsDir, 'fstack-qa'))).toBe(true);
    expect(fs.existsSync(path.join(skillsDir, 'fstack-ship'))).toBe(true);
    expect(fs.existsSync(path.join(skillsDir, 'fstack-review'))).toBe(true);
    expect(output).toContain('fstack-');
  });

  // Test 12: flat symlinks when skill_prefix=false
  test('creates flat symlinks when skill_prefix=false', () => {
    setupMockInstall(['qa', 'ship', 'review']);
    run(`${path.join(installDir, 'bin', 'fstack-config')} set skill_prefix false`, {
      FSTACK_INSTALL_DIR: installDir,
      FSTACK_SKILLS_DIR: skillsDir,
    });
    const output = run(`${path.join(installDir, 'bin', 'fstack-relink')}`, {
  ...
  // e.g., `qa -> fstack/qa` gets discovered as "fstack-qa", not "qa".
  ...
  test('unprefixed skills are real directories with SKILL.md symlinks, not dir symlinks', () => {
    ...
    run(`${path.join(installDir, 'bin', 'fstack-relink')}`, {
      FSTACK_INSTALL_DIR: installDir,
      FSTACK_SKILLS_DIR: skillsDir,
    });
  ...
  test('prefixed skills are real directories with SKILL.md symlinks, not dir symlinks', () => {
    ...
    for (const skill of ['fstack-qa', 'fstack-ship']) {
      const skillPath = path.join(skillsDir, skill);
      const skillMdPath = path.join(skillPath, 'SKILL.md');
      expect(fs.lstatSync(skillPath).isDirectory()).toBe(true);
      expect(fs.lstatSync(skillPath).isSymbolicLink()).toBe(false);
  ...
  // FIRST INSTALL: --no-prefix must create ONLY flat names, zero fstack-* pollution
  test('first install --no-prefix: only flat names exist, zero fstack-* entries', () => {
    setupMockInstall(['qa', 'ship', 'review', 'plan-ceo-review', 'fstack-upgrade']);
    ...
    // Expected: qa, ship, review, plan-ceo-review, fstack-upgrade (its real name)
    expect(entries.sort()).toEqual(['fstack-upgrade', 'plan-ceo-review', 'qa', 'review', 'ship']);
    // No fstack-qa, fstack-ship, fstack-review, fstack-plan-ceo-review
    const leaked = entries.filter(e => e.startsWith('fstack-') && e !== 'fstack-upgrade');
    expect(leaked).toEqual([]);
  });

  // FIRST INSTALL: --prefix must create ONLY fstack-* names, zero flat-name pollution
  test('first install --prefix: only fstack-* entries exist, zero flat names', () => {
    setupMockInstall(['qa', 'ship', 'review', 'plan-ceo-review', 'fstack-upgrade']);
    run(`${path.join(installDir, 'bin', 'fstack-config')} set skill_prefix true`, {
      FSTACK_INSTALL_DIR: installDir,
      FSTACK_SKILLS_DIR: skillsDir,
    ...
    const entries = fs.readdirSync(skillsDir);
    // Expected: fstack-qa, fstack-ship, fstack-review, fstack-plan-ceo-review, fstack-upgrade
    expect(entries.sort()).toEqual([
      'fstack-plan-ceo-review', 'fstack-qa', 'fstack-review', 'fstack-ship', 'fstack-upgrade',
    ]);
    // No unprefixed qa, ship, review, plan-ceo-review
    const leaked = entries.filter(e => !e.startsWith('fstack-'));
    expect(leaked).toEqual([]);
  });
  ...
  test('non-TTY first install defaults to flat names via relink', () => {
    ...
    // fstack-relink reads config; on fresh install config returns empty → defaults to false
    run(`${path.join(installDir, 'bin', 'fstack-relink')}`, {
      FSTACK_INSTALL_DIR: installDir,
      FSTACK_SKILLS_DIR: skillsDir,
    });
  ...
  // SWITCH: prefix → no-prefix must clean up ALL fstack-* entries
  test('switching prefix to no-prefix removes all fstack-* entries completely', () => {
    setupMockInstall(['qa', 'ship', 'review', 'plan-ceo-review', 'fstack-upgrade']);
    ...
    expect(entries.filter(e => !e.startsWith('fstack-'))).toEqual([]);
    ...
    // Only flat names + fstack-upgrade (its real name)
    expect(entries.sort()).toEqual(['fstack-upgrade', 'plan-ceo-review', 'qa', 'review', 'ship']);
    const leaked = entries.filter(e => e.startsWith('fstack-') && e !== 'fstack-upgrade');
    expect(leaked).toEqual([]);
  });
  ...
  test('switching no-prefix to prefix removes all flat entries completely', () => {
    setupMockInstall(['qa', 'ship', 'review', 'fstack-upgrade']);
    ...
    let entries = fs.readdirSync(skillsDir);
    expect(entries.filter(e => e.startsWith('fstack-') && e !== 'fstack-upgrade')).toEqual([]);
    ...
    // Only fstack-* names
    expect(entries.sort()).toEqual([
      'fstack-qa', 'fstack-review', 'fstack-ship', 'fstack-upgrade',
    ]);
    const leaked = entries.filter(e => !e.startsWith('fstack-'));
    expect(leaked).toEqual([]);
  });
  ...
  test('cleans up stale symlinks from opposite mode', () => {
    ...
    expect(fs.existsSync(path.join(skillsDir, 'qa'))).toBe(true);
    expect(fs.existsSync(path.join(skillsDir, 'fstack-qa'))).toBe(false);
  });

  // Test 14: error when install dir missing
  test('prints error when install dir missing', () => {
    const output = run(`${BIN}/fstack-relink`, {
      FSTACK_INSTALL_DIR: '/nonexistent/path/fstack',
      FSTACK_SKILLS_DIR: '/nonexistent/path/skills',
    }, true);
    expect(output).toContain('setup');
  });

  // Test: fstack-upgrade does NOT get double-prefixed
  test('does not double-prefix fstack-upgrade directory', () => {
    setupMockInstall(['qa', 'ship', 'fstack-upgrade']);
    run(`${path.join(installDir, 'bin', 'fstack-config')} set skill_prefix true`, {
      FSTACK_INSTALL_DIR: installDir,
      FSTACK_SKILLS_DIR: skillsDir,
    });
    ...
    // fstack-upgrade should keep its name, NOT become fstack-fstack-upgrade
    expect(fs.existsSync(path.join(skillsDir, 'fstack-upgrade'))).toBe(true);
    expect(fs.existsSync(path.join(skillsDir, 'fstack-fstack-upgrade'))).toBe(false);
    // Regular skills still get prefixed
    expect(fs.existsSync(path.join(skillsDir, 'fstack-qa'))).toBe(true);
  });

  // Test 15: fstack-config set skill_prefix triggers relink
  test('fstack-config set skill_prefix triggers relink', () => {
    setupMockInstall(['qa', 'ship']);
    // Run fstack-config set which should auto-trigger relink
    run(`${path.join(installDir, 'bin', 'fstack-config')} set skill_prefix true`, {
      FSTACK_INSTALL_DIR: installDir,
      FSTACK_SKILLS_DIR: skillsDir,
    });
...
describe('upgrade migrations', () => {
  const MIGRATIONS_DIR = path.join(ROOT, 'fstack-upgrade', 'migrations');
  ...
  test('v0.15.2.0 migration runs fstack-relink', () => {
    const content = fs.readFileSync(path.join(MIGRATIONS_DIR, 'v0.15.2.0.sh'), 'utf-8');
    expect(content).toContain('fstack-relink');
  });
  ...
  test('v0.15.2.0 migration fixes stale directory symlinks', () => {
    ...
    run(`${path.join(installDir, 'bin', 'fstack-config')} set skill_prefix false`, {
      FSTACK_SETUP_RUNNING: '1',
    });
    // Verify old state: symlinks
    expect(fs.lstatSync(path.join(skillsDir, 'qa')).isSymbolicLink()).toBe(true);

    // Run the migration (it calls fstack-relink internally)
...
describe('fstack-patch-names (#620/#578)', () => {
  ...
  test('prefix=true patches name: field in SKILL.md', () => {
    ...
    // Verify name: field is patched with fstack- prefix
    expect(readSkillName(path.join(installDir, 'qa'))).toBe('fstack-qa');
    expect(readSkillName(path.join(installDir, 'ship'))).toBe('fstack-ship');
    expect(readSkillName(path.join(installDir, 'review'))).toBe('fstack-review');
  });
  ...
  test('fstack-upgrade name: not double-prefixed', () => {
    setupMockInstall(['qa', 'fstack-upgrade']);
    run(`${path.join(installDir, 'bin', 'fstack-config')} set skill_prefix true`, {
      FSTACK_INSTALL_DIR: installDir,
      FSTACK_SKILLS_DIR: skillsDir,
    });
    ...
    // fstack-upgrade should keep its name, NOT become fstack-fstack-upgrade
    expect(readSkillName(path.join(installDir, 'fstack-upgrade'))).toBe('fstack-upgrade');
