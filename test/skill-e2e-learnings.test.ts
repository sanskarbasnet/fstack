...
describeIfSelected('Learnings E2E', ['learnings-show'], () => {
  ...
  beforeAll(() => {
    ...
    for (const script of ['fstack-learnings-search', 'fstack-learnings-log', 'fstack-slug']) {
      fs.copyFileSync(path.join(ROOT, 'bin', script), path.join(binDir, script));
      fs.chmodSync(path.join(binDir, script), 0o755);
    }

    // Seed learnings JSONL — slug must match what fstack-slug computes.
    // With no git remote, fstack-slug falls back to basename(workDir).
...
- Replace any references to ~/.claude/skills/fstack/bin/fstack-slug with ./bin/fstack-slug.
