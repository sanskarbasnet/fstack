...
// Eval result collector — accumulates test results, writes to ~/.fstack-dev/evals/ on finalize
...
function dumpOutcomeDiagnostic(dir: string, label: string, report: string, judgeResult: any) {
  try {
    const transcriptDir = path.join(dir, '.fstack', 'test-transcripts');
...
Report whether it worked.`,
  ...
  testIfSelected('skillmd-no-local-binary', async () => {
    // Create a tmpdir with no browse binary — no local .claude/skills/fstack/browse/dist/browse
...
Report the exact output. Do NOT try to fix or install anything — just report what you see.`,
    ...
    // On dev machines with fstack installed globally, the fallback path
    // ~/.claude/skills/fstack/browse/dist/browse exists, so we get READY.
...
Report the exact output — either "READY: <path>" or "NEEDS_SETUP".`,
  ...
  testIfSelected('session-awareness', async () => {
    ...
    const result = await runSkillTest({
      prompt: `You are running a fstack skill. The session preamble detected _SESSIONS=4 (the user has 4 fstack windows open).
...
Write your report to ${qaOnlyDir}/qa-reports/qa-only-report.md`,
    ...
    const statusLines = gitStatus.stdout.toString().trim().split('\n').filter(
      (l: string) => l.trim() && !l.includes('.prompt-tmp') && !l.includes('.fstack/') && !l.includes('qa-reports/'),
    );
...
`);
    ...
    projectDir = path.join(os.homedir(), '.fstack', 'projects', 'test-project');
  ...
  test('/plan-eng-review writes test-plan artifact to ~/.fstack/projects/', async () => {
...
describeIfSelected('fstack-upgrade E2E', ['fstack-upgrade-happy-path'], () => {
  ...
  testIfSelected('fstack-upgrade-happy-path', async () => {
    const mockFstack = path.join(upgradeDir, '.claude', 'skills', 'fstack');
    const result = await runSkillTest({
...
IMPORTANT: The install directory is at ./.claude/skills/fstack — use that exact path.`,
    ...
    recordE2E('/fstack-upgrade happy path', 'fstack-upgrade E2E', result, {
