...
Thanks for wanting to make fstack better. Whether you're fixing a typo in a skill prompt or building an entirely new workflow, this guide will get you up and running fast.

## Quick start

fstack skills are Markdown files that Claude Code discovers from a `skills/` directory. Normally they live at `~/.claude/skills/fstack/` (your global install). But when you're developing fstack itself, you want Claude Code to use the skills *in your working tree* — so edits take effect instantly without copying or deploying anything.

That's what dev mode does. It symlinks your repo into the local `.claude/skills/` directory so Claude Code reads skills straight from your checkout.

```bash
git clone https://github.com/garrytan/gstack.git && cd fstack
...
fstack automatically learns from failures. At the end of every skill session, the agent
reflects on what went wrong (CLI errors, wrong approaches, project quirks) and logs
operational learnings to `~/.fstack/projects/{slug}/learnings.jsonl`. Future sessions
surface these learnings automatically, so fstack gets smarter on your codebase over time.

No setup needed. Learnings are logged automatically. View them with `/learn`.

### The contributor workflow

1. **Use fstack normally** — operational learnings are captured automatically
2. **Check your learnings:** `/learn` or `ls ~/.fstack/projects/*/learnings.jsonl`
3. **Fork and clone fstack** (if you haven't already)
4. **Symlink your fork into the project where you hit the bug:**
   ```bash
   # In your core project (the one where fstack annoyed you)
   ln -sfn /path/to/your/fstack-fork .claude/skills/fstack
   cd .claude/skills/fstack && bun install && bun run build && ./setup
   ```
   Setup creates per-skill directories with SKILL.md symlinks inside (`qa/SKILL.md -> fstack/qa/SKILL.md`)
   and asks your prefix preference. Pass `--no-prefix` to skip the prompt and use short names.
5. **Fix the issue** — your changes are live immediately in this project
6. **Test by actually using fstack** — do the thing that annoyed you, verify it's fixed
7. **Open a PR from your fork**

This is the best way to contribute: fix fstack while doing your real work, in the
project where you actually felt the pain.

### Session awareness

When you have 3+ fstack sessions open simultaneously, every question tells you which project, which branch, and what's happening. No more staring at a question thinking "wait, which window is this?" The format is consistent across all skills.

## Working on fstack inside the fstack repo

When you're editing fstack skills and want to test them by actually using fstack
...
```
fstack/                          <- your working tree
├── .claude/skills/              <- created by dev-setup (gitignored)
│   ├── fstack -> ../../         <- symlink back to repo root
│   ├── review/                  <- real directory (short name, default)
│   │   └── SKILL.md -> fstack/review/SKILL.md
│   ├── ship/                    <- or fstack-review/, fstack-ship/ if --prefix
│   │   └── SKILL.md -> fstack/ship/SKILL.md
...
under `fstack/`. Names depend on your prefix setting (`~/.fstack/config.yaml`).
Short names (`/review`, `/ship`) are the default. Run `./setup --prefix` if you
prefer namespaced names (`/fstack-review`, `/fstack-ship`).
...
When E2E tests run, they produce machine-readable artifacts in `~/.fstack-dev/`:
...
Artifacts are never cleaned up — they accumulate in `~/.fstack-dev/` for post-mortem debugging and trend analysis.
...
fstack's Writing Style section (injected into every tier-≥2 skill's preamble)
...
fstack generates SKILL.md files for 8 hosts from one set of `.tmpl` templates.
...
|--------|---------------------------|
| Output directory | `{skill}/SKILL.md` vs `.agents/skills/fstack-{skill}/SKILL.md` |
| Frontmatter | Full (name, description, hooks, version) vs minimal (name + description) |
| Paths | `~/.claude/skills/fstack` vs `$FSTACK_ROOT` |
...
- **Dev mode shadows your global install.** Project-local skills take priority over `~/.claude/skills/fstack`. `bin/dev-teardown` restores the global one.
...
**This is the recommended way to develop fstack.** Symlink your fstack checkout
...
```bash
# In your core project (not the fstack repo)
ln -sfn /path/to/your/fstack-checkout .claude/skills/fstack
```

### Step 2: Run setup to create per-skill symlinks

The `fstack` symlink alone isn't enough. Claude Code discovers skills through
individual top-level directories (`qa/SKILL.md`, `ship/SKILL.md`, etc.), not through
the `fstack/` directory itself. Run `./setup` to create them:

```bash
cd .claude/skills/fstack && bun install && bun run build && ./setup
```

Setup will ask whether you want short names (`/qa`) or namespaced (`/fstack-qa`).
Your choice is saved to `~/.fstack/config.yaml` and remembered for future runs.
...
Remove the project-local symlink. Claude Code falls back to `~/.claude/skills/fstack/`:

```bash
rm .claude/skills/fstack
```

The per-skill directories (`qa/`, `ship/`, etc.) contain SKILL.md symlinks that point
to `fstack/...`, so they'll resolve to the global install automatically.

### Switching prefix mode

If you installed fstack with one prefix setting and want to switch:

```bash
cd .claude/skills/fstack && ./setup --no-prefix   # switch to /qa, /ship
cd .claude/skills/fstack && ./setup --prefix       # switch to /fstack-qa, /fstack-ship
```
...
- Renamed or moved config keys in `~/.fstack/config.yaml`
- Need to delete orphaned files from a previous version
- Changed the format of `~/.fstack/` state files
...
1. Create `fstack-upgrade/migrations/v{VERSION}.sh` where `{VERSION}` matches
   the VERSION file for the release that needs the fix.
2. Make it executable: `chmod +x fstack-upgrade/migrations/v{VERSION}.sh`
...
"$SCRIPT_DIR/bin/fstack-relink" 2>/dev/null || true
```

### How it runs

During `/fstack-upgrade`, after `./setup` completes (Step 4.75), the upgrade
skill scans `fstack-upgrade/migrations/` and runs every `v*.sh` script whose
...
verifies that all migration scripts in `fstack-upgrade/migrations/` are
