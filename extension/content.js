...
function showStatusPill(connected, refs) {
  ...
  if (!statusPill) {
    ...
    statusPill.id = 'fstack-status-pill';
  ...
  statusPill.innerHTML = `<span class="fstack-pill-dot"></span> fstack${refText}`;
...
function ensureContainer() {
  ...
  overlayContainer.id = 'fstack-ref-overlays';
...
function renderRefBadges(refs) {
  ...
  for (const ref of refs) {
    ...
    badge.className = 'fstack-ref-badge';
...
function renderRefPanel(refs) {
  ...
  panel.className = 'fstack-ref-panel';

  const header = document.createElement('div');
  header.className = 'fstack-ref-panel-header';
  header.textContent = `fstack refs (${refs.length})`;
  header.style.cssText = 'pointer-events: auto; cursor: move;';
  panel.appendChild(header);

  const list = document.createElement('div');
  list.className = 'fstack-ref-panel-list';
  for (const ref of refs.slice(0, 30)) { // Show max 30 in panel
    const row = document.createElement('div');
    row.className = 'fstack-ref-panel-row';
    const idSpan = document.createElement('span');
    idSpan.className = 'fstack-ref-panel-id';
    idSpan.textContent = ref.ref;
    const roleSpan = document.createElement('span');
    roleSpan.className = 'fstack-ref-panel-role';
    roleSpan.textContent = ref.role;
    const nameSpan = document.createElement('span');
    nameSpan.className = 'fstack-ref-panel-name';
  ...
  if (refs.length > 30) {
    ...
    more.className = 'fstack-ref-panel-more';
...
// Do NOT dispatch fstack-extension-ready here — the extension being loaded
...
chrome.runtime.onMessage.addListener((msg) => {
  ...
  if (msg.type === 'sidebarOpened') {
    document.dispatchEvent(new CustomEvent('fstack-extension-ready'));
    return;
  }
