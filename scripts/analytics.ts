#!/usr/bin/env bun
/**
 * analytics — CLI for viewing fstack skill usage statistics.
 *
 * Reads ~/.fstack/analytics/skill-usage.jsonl and displays:
...
const ANALYTICS_FILE = path.join(os.homedir(), '.fstack', 'analytics', 'skill-usage.jsonl');
...
export function formatReport(events: AnalyticsEvent[], period: string = 'all'): string {
  ...
  const lines: string[] = [];
  lines.push('fstack skill usage analytics');
