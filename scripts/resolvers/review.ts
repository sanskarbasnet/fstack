 ...
 *
 * Review logs are stored locally at ~/.fstack/reviews/review-log.jsonl.
...
echo '{"skill":"${_ctx.skillName}","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.fstack/analytics/spec-review.jsonl 2>/dev/null || true
\`\`\`
...
SLUG=$(~/.claude/skills/fstack/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-' || echo 'no-branch')
DESIGN=$(ls -t ~/.fstack/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t ~/.fstack/projects/$SLUG/*-design-*.md 2>/dev/null | head -1)
...
\`\`\`bash
CODEX_PROMPT_FILE=$(mktemp /tmp/fstack-codex-oh-XXXXXXXX.txt)
\`\`\`
...
**Persist the result:**
\`\`\`bash
~/.claude/skills/fstack/bin/fstack-review-log '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
\`\`\`
