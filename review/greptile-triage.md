...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.claude/skills/fstack/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
PROJECT_HISTORY="$HOME/.fstack/projects/$REMOTE_SLUG/greptile-history.md"
```
...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.claude/skills/fstack/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
mkdir -p "$HOME/.fstack/projects/$REMOTE_SLUG"
mkdir -p ~/.fstack
```

Append one line per triage outcome to **both** files (per-project for suppressions, global for retro):
- `~/.fstack/projects/$REMOTE_SLUG/greptile-history.md` (per-project)
- `~/.fstack/greptile-history.md` (global aggregate)
