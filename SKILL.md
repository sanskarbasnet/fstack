...
description: |
  ...
  test a site, verify a deployment, dogfood a user flow, or file a bug with screenshots. (fstack)
allowed-tools:
  - Bash
  - Read
  - AskUserQuestion
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"fstack","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.fstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/fstack/bin/fstack-timeline-log '{"skill":"fstack","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
If `PROACTIVE` is `false`: do NOT proactively invoke or suggest other fstack skills during
this session. Only run skills the user explicitly invokes. This preference persists across
sessions via `fstack-config`.
...
- User asks to upgrade fstack → invoke `/fstack-upgrade`
...
- User asks to launch a real browser for QA, "open the browser" → invoke `/open-fstack-browser`
...
- User asks what fstack has learned, "show learnings" → invoke `/learn`
...
If the user opts out of suggestions, run `fstack-config set proactive false`.
If they opt back in, run `fstack-config set proactive true`.

# fstack browse: QA Testing & Dogfooding
...
`$B` is the browse binary (resolved from `$_ROOT/.claude/skills/fstack/browse/dist/browse` or `~/.claude/skills/fstack/browse/dist/browse`).
